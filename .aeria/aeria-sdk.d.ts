...
declare type MirrorDescriptions = {
  ...
  user: {
    $id: "user";
    icon: "users";
    required: ["name", "roles", "email"];
    form: ["name", "active", "roles", "email", "phone_number", "picture_file"];
    indexes: ["name"];
    properties: {
      name: {
        type: "string";
      ...
      roles: {
        ...
        items: {
          type: "string";
          enum: ["root", "supervisor", "customer"];
          values: ["root", "supervisor", "customer"];
        };
        uniqueItems: true;
      };
      email: {
    ...
    filters: ["name", "roles", "email", "phone_number"];
    table: ["name", "roles", "picture_file", "active", "updated_at"];
    tableMeta: ["email"];
    formLayout: {
      fields: {
        given_name: {
...
declare module "aeria-sdk" {
      ...
      ? MirrorRouter[Route][Method] extends infer Contract
        ? Contract extends
          ...
          ? MakeEndpoint<
              ...
              RoutePayload extends {} ? InferProperty<RoutePayload> : undefined
            >
        ...
        ? Fn extends keyof CollectionFunctionsSDK<any>
          ? Record<
              ...
              Record<
                Fn,
                {
                  POST: CollectionFunctionsSDK<
                    SchemaWithId<MirrorDescriptions[Coll]>
                  >[Fn];
                }
              >
            >
  ...
  type TopLevelAeria = ((bearerToken?: string) => TopLevelObject & Endpoints) &
    TopLevelObject &
    Endpoints;
